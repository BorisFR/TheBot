...
FaceBehavior::FaceBehavior(TheBotFace &face)
    : _face(face), Timer(500)
{
    Timer.Start();
    Clear();
...
eEmotions FaceBehavior::GetRandomEmotion()
{
    ...
    if (sum_of_weight == 0)
    {
        uint8_t choose = random(0, eEmotions::EMOTIONS_COUNT);
        return (eEmotions)choose;
...
void FaceBehavior::Update()
{
    ...
    {
        Timer.Reset();
        eEmotions newEmotion = eEmotions::EMOTIONS_COUNT;
        do
        {
            newEmotion = GetRandomEmotion();
        } while (CurrentEmotion == newEmotion);
